...
exports.updateRoom = async(req, res)=>{
    try{
        ...
        const userId = req.user.id;
        const images = req.files;
        const { title, type, beds, price, rooms, hotelId, roomId, deleteImage } = req.body;
        ...
        if(!hotel.roooms.includes(roomId)){
            throw customError("This Room doesn't belongs to selected hotel", 401);
        }
        if(!title && !type && !beds && !price && !rooms && deleteImage.length == 0 && images.length == 0){
        ...
        if(deleteImage.length !== 0){
            const updatedImageURLs = room.imageURL.filter(url => !deleteImage.includes(url));
            room.imageURL = updatedImageURLs;
        }

        if(title){  room.roomTitle = title  }
        if(type){  room.roomType = type  }
        if(beds){  room.numberOfBed = beds  }
        if(price){  room.price = price  }
        if(rooms){  room.numberOfRooms = rooms  }
